...
# dsp_functions.py
import numpy as np
from scipy.signal import butter, filtfilt, lfilter, medfilt, find_peaks, decimate
...
def r_peak_detection(signal, fs=250, height=None, distance=None, **kwargs):
    ...
    sig = np.array(signal)
    distance = distance or int(0.25 * fs)  # ~250ms refractory period
    peaks, _ = find_peaks(sig, height=height, distance=distance)
    return peaks.tolist()

def hr_calc(peaks, fs=250, **kwargs):
